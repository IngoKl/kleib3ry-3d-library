...
- **[modes.md](modes.md)** — the two ways to run it: the desktop app, and the
  container that serves a library folder to a browser. What is the same, what
  differs, and what hosted mode deliberately does not do. Start here if you are
...
- **[architecture.md](architecture.md)** — what the pieces are, which way the
  dependencies point, and why the non-obvious decisions were made. Start here.
...
- **[../CLAUDE.md](../CLAUDE.md)** — the short form of the architecture, for an
  agent working in the repo. A summary of `architecture.md`, not a second source
  of truth: if the two disagree, `architecture.md` wins.
